	...
	for partitionID := range pmap {
		...
		for msg := range partition.Messages() {
			...
			if string(msg.Value) != check.message {
				fmt.Printf("msg values not equal: partition: %d: offset: %d", msg.Partition, msg.Offset)
			}
			if msg.Offset != check.offset {
				fmt.Printf("msg offsets not equal: partition: %d: offset: %d", msg.Partition, msg.Offset)
			}
